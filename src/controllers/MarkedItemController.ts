...
export const getMarkedItemsByUserId = async (
  ...
  try {
    ...
    const response: QueryResult = await pool.query(
      'SELECT mi.*, i.num_reviews, i.rating, i.food_name, i.brand_name, i.image_url, i.nix_item_id FROM markedItems mi JOIN items i ON mi.item_id=i.id WHERE user_id = $1',
      [userId]
    );
